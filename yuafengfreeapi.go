...
func fetchLyricFromYaohu(songName, artistName, dirPath string) bool {
	apiKey := os.Getenv("YAOHU_API_KEY")
	...
	if err := json.Unmarshal(body, &qqResp); err != nil { return false }
	if qqResp.Code != 200 || qqResp.Data.Viplrc == "" { return false }
	...
	if lyricText == "" || len(lyricText) < 10 { return false }
...
func getRemoteMusicURLOnly(song, singer string) string {
	...
	for _, host := range apiHosts {
		for _, source := range sources {
			...
			if err != nil { continue }
			var response YuafengAPIFreeResponse
			if err := json.Unmarshal(body, &response); err != nil { continue }
			if response.Data.Music != "" {
